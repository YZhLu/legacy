<script lang="ts">
	...
	onMount(() => {
		...
		const _options = shuffleArray(questData[rd].options);
		...
		if (optionsType === 'latex') {
			options = _options.map((option) => parser.parse(option as string));
		} else {
			...
			<div class="flex flex-col gap-2 text-base md:text-2xl">
				{#each options as option}
					<button
						...
						onclick={() => hdlCheck(option)}
						class="hover:bg-primary-500 flex w-full gap-4 rounded-xl p-4 md:text-lg"
					>
