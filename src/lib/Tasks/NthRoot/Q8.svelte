<script lang="ts">
	...
	export function init() {
		let ans, displayExpression, num, den;
		let base_num = [2, 3, 4, 5, 6, 7, 8][Math.floor(random() * 7)];
		let base_den = ['a', 'b', 'c', 11, 13, 17, 19][Math.floor(random() * 7)];

		let exp_num = [1, 3, 5, 7, 9][Math.floor(random() * 5)];
		let exp_den = ['q', 'r', 's', 11, 13, 17, 19][Math.floor(random() * 7)];

		let index = [2, 3, 5, 7][Math.floor(random() * 4)];
		...
		if (exp_num === 1) {
			...
			den = `${base_den}^{${exp_den}}`;
		}

		if (index === 2) {
			displayExpression = `\\sqrt{\\frac{${num}}{${den}}}=`;
			ans = `\\frac{\\sqrt{${num}}}{\\sqrt{${den}}}`;
		} else {
			displayExpression = `\\sqrt[${index}]{\\frac{${num}}{${den}}}=`;
			ans = `\\frac{\\sqrt[${index}]{${num}}}{\\sqrt[${index}]{${den}}}`;
		}
