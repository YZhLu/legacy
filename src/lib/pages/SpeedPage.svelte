<script lang="ts">
	...
	// import { speedChapters } from '../../routes/api/student-[studentId]/speed';
	...
	let value = $state([speedChapters[questLevel].title]); ///* [questData.books[book].content.speedChapters[questLevel].title] */);

	let usingApi = baseUrl.includes('api');
</script>

<div class="flex flex-col items-center justify-center pt-4">
	<span class="flex pb-4 text-3xl font-bold"><!-- 9° ano --><Icon icon="bytesize:lightning" width="38" height="38" /> Tempo</span>
	<span class="pb-4 text-center text-xl font-bold">Vamos treinar velocidade!</span>
	<button
	...
	<section class="py-10">
		...
		<Accordion {value} onValueChange={(e) => (value = e.value)} multiple width="w-80 md:w-[550px]">
			...
			{/snippet}
			{#each speedChapters as speedChapter, speedChapterIndex}
				<Accordion.Item value={speedChapter.title} classes="md:text-lg">
					<!-- Control -->
					...
					{#snippet control()}
						<span class="grid" style="font-family: Nunito;">
							<span class="foont-bild text-surface-500 flex text-xs md:text-sm">
								Estágio {speedChapterIndex + 1}
							</span>
							<p class=" flex items-baseline text-lg font-bold">
								<!-- <Flame size={18} /> -->{speedChapter.title}
							</p>
						</span>
					{/snippet}
					...
					{#snippet panel()}
						<div class="grid gap-4 py-2" style="font-family: Nunito;">
							{#each speedChapter.sections as section, sectionIndex}
								<div class="md:text-lg">
									...
									>
										{#each section.drills as quest, questIndex}
											{#if isTeacherMode}
