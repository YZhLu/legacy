		...
		try {
			...
			for (const [key, value] of Object.entries(rawScope)) {
				if (typeof value === 'object') {
					...
					// Gera e armazena o valor imediatamente
			...
			for (const [key, value] of Object.entries(rawScope)) {
				if (typeof value === 'string') {
					const expression = value.replace(/\|([a-z])\|/g, (match, varName) => {
 ...
 */
	...
	private formatToLatexFraction(value: number): { n: any; d: any } {
			...
			for (const [key, value] of Object.entries(rawScope)) {
				if (typeof value === 'string') {
					...
					if (methodMatch && helperMethods[methodMatch[1]]) {
						...
						const expression = methodMatch[2].replace(/\|([a-z])\|/g, (match, varName) => {
							if (this.generatedValues[varName] === undefined) {
								throw new Error(`Variable ${varName} referenced before definition`);
							}
							return this.generatedValues[varName].toString();
	...
	private formatExpression(exp: string): string {
		return (
			exp
				//.replace(/[+-]?\s*0(?:x[\^]?\d*)?(?=\s|$|\+|-)/g, '')
				/* .replace(/([+-]?\s*)(0(?:x[\^]?\d*)?(?=\s|$|\+|-))/g, (match, term) => {
					const varName = term.includes('x') ? 'b' : 'c'; // Determine if it's x term or constant
					return this.scope[varName]?.noNull ? '' : match;
				}) */
				//.replace(/1(?=x)/g, ' ')
				.replace(/(?<!^)\s+(\d+)/g, ' + $1')
				.replace(/\b1(?=x(?:\^|[^0-9]))/g, '')
	...
	/* parse(expression: string): string {
		...
		result = result.replace(varPattern, (match, varName) => {
			...
			return value.toString(); //value == 0? '':value.toString();
		});
 ...
 */
	...
	/* parse(expression: string): string {
		...
		result = result.replace(varPattern, (match, varName) => {
			...
			if (this.generatedValues[`${varName}n`] !== undefined && 
				...
				// Otherwise return LaTeX fraction
	...
	parse(expression: string): string {
		...
		result = result.replace(varPattern, (match, varName) => {
			...
			if (
				...
				//console.log(this.generatedValues[`${varName}s`])
